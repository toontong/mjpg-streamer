...
int uvc_move(struct coord *cent, struct control_data *cd) {
	...
	if (cd->minmaxfound == 1) {
		...
		if (move_y_degrees < 0
				&& (cd->tiltmin - cd->tilt_angle) > move_y_degrees)
			move_y_degrees = (cd->tiltmin - cd->tilt_angle);

		if (move_y_degrees > 0
				&& (cd->tiltmax - cd->tilt_angle) < move_y_degrees)
...
static int handle_get(int client_socket, const char* url,
	...
	if (*url == '/') {
		...
		if (ssconfret == 2) {
			...
			if (!strcmp(command, "control")) {
				send_control_template(client_socket, click_control,
						cd->control_port, cd->stream_port);
				send_template_end_client(client_socket);
			} else {
